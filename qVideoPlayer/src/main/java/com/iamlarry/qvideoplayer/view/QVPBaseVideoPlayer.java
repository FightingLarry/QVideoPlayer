...
import static com.iamlarry.qvideoplayer.utils.CommonUtil.getActionBarHeight;
import static com.iamlarry.qvideoplayer.utils.CommonUtil.getStatusBarHeight;
import static com.iamlarry.qvideoplayer.utils.CommonUtil.hideNavKey;
import static com.iamlarry.qvideoplayer.utils.CommonUtil.hideSupportActionBar;
import static com.iamlarry.qvideoplayer.utils.CommonUtil.showNavKey;
import static com.iamlarry.qvideoplayer.utils.CommonUtil.showSupportActionBar;
...
public abstract class QVPBaseVideoPlayer extends QVPVideoControlView {
    ...
    public QVPBaseVideoPlayer(Context context, Boolean fullFlag) {
        super(context, fullFlag);
    }

    public QVPBaseVideoPlayer(Context context) {
        super(context);
    }

    public QVPBaseVideoPlayer(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public QVPBaseVideoPlayer(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void cloneParams(QVPBaseVideoPlayer from, QVPBaseVideoPlayer to) {
        ...
        to.mIQVPVideoProgressListener = from.mIQVPVideoProgressListener;
        ...
        to.mIVideoAllCallBack = from.mIVideoAllCallBack;
    ...
    private void pauseFullCoverLogic() {
        if (mCurrentState == QVPVideoPlayer.CURRENT_STATE_PAUSE && mTextureView != null
     ...
     * 全屏的暂停返回的时候返回页面不黑色
     */
    private void pauseFullBackCoverLogic(QVPBaseVideoPlayer gsyVideoPlayer) {
        //如果是暂停状态
        if (gsyVideoPlayer.mCurrentState == QVPVideoPlayer.CURRENT_STATE_PAUSE
     ...
     * 全屏
     */
    protected void resolveFullVideoShow(Context context, final QVPBaseVideoPlayer gsyVideoPlayer, final FrameLayout frameLayout) {
        ...
        if (mIVideoAllCallBack != null) {
            ...
            mIVideoAllCallBack.onEnterFullscreen(mOriginUrl, mTitle, gsyVideoPlayer);
        }
    ...
    protected void resolveNormalVideoShow(View oldF, ViewGroup vp, QVPVideoPlayer gsyVideoPlayer) {
        ...
        if (mIVideoAllCallBack != null) {
            ...
            mIVideoAllCallBack.onQuitFullscreen(mOriginUrl, mTitle, this);
        }
    ...
    protected void clearFullscreenLayout() {
        ...
        if (oldF != null) {
            ...
            QVPVideoPlayer gsyVideoPlayer = (QVPVideoPlayer) oldF;
    ...
    protected void backToNormal() {
        ...
        final View oldF = vp.findViewById(getFullId());
        final QVPVideoPlayer gsyVideoPlayer;
        if (oldF != null) {
            gsyVideoPlayer = (QVPVideoPlayer) oldF;
            //如果暂停了
    ...
    protected Runnable mCheckoutTask = new Runnable() {
        @Override
        public void run() {
            QVPVideoPlayer gsyVideoPlayer = getFullWindowPlayer();
            if (gsyVideoPlayer != null
    ...
    public QVPBaseVideoPlayer startWindowFullscreen(final Context context, final boolean actionBar, final boolean statusBar) {
        ...
        try {
            QVPBaseVideoPlayer.this.getClass().getConstructor(Context.class, Boolean.class);
        } catch (Exception e) {
        ...
        try {
            //通过被重载的不同构造器来选择
            Constructor<QVPBaseVideoPlayer> constructor;
            final QVPBaseVideoPlayer gsyVideoPlayer;
            if (!hadNewConstructor) {
                constructor = (Constructor<QVPBaseVideoPlayer>) QVPBaseVideoPlayer.this.getClass().getConstructor(Context.class);
                gsyVideoPlayer = constructor.newInstance(getActivityContext());
            } else {
                constructor = (Constructor<QVPBaseVideoPlayer>) QVPBaseVideoPlayer.this.getClass().getConstructor(Context.class, Boolean.class);
     ...
     */
    @SuppressWarnings("ResourceType, unchecked")
    public QVPBaseVideoPlayer showSmallVideo(Point size, final boolean actionBar, final boolean statusBar) {
        ...
        try {
            Constructor<QVPBaseVideoPlayer> constructor = (Constructor<QVPBaseVideoPlayer>) QVPBaseVideoPlayer.this.getClass().getConstructor(Context.class);
            QVPBaseVideoPlayer gsyVideoPlayer = constructor.newInstance(getActivityContext());
            ...
            if (mIVideoAllCallBack != null) {
                ...
                mIVideoAllCallBack.onEnterSmallWidget(mOriginUrl, mTitle, gsyVideoPlayer);
            }
    ...
    public void hideSmallVideo() {
        final ViewGroup vp = getViewGroup();
        QVPVideoPlayer gsyVideoPlayer = (QVPVideoPlayer) vp.findViewById(getSmallId());
        removeVideo(vp, getSmallId());
        mCurrentState = getGSYVideoManager().getLastState();
        if (gsyVideoPlayer != null) {
            cloneParams(gsyVideoPlayer, this);
        ...
        if (mIVideoAllCallBack != null) {
            ...
            mIVideoAllCallBack.onQuitSmallWidget(mOriginUrl, mTitle, this);
        }
    }
     ...
     */
    @SuppressWarnings("ResourceType")
    public QVPVideoPlayer getFullWindowPlayer() {
     ...
     */
    @SuppressWarnings("ResourceType")
    public QVPVideoPlayer getSmallWindowPlayer() {
        ViewGroup vp = (ViewGroup) (CommonUtil.scanForActivity(getContext())).findViewById(Window.ID_ANDROID_CONTENT);
        final View small = vp.findViewById(getSmallId());
        QVPVideoPlayer gsyVideoPlayer = null;
        if (small != null) {
            gsyVideoPlayer = (QVPVideoPlayer) small;
        }
        return gsyVideoPlayer;
    }
     ...
     * 获取当前长在播放的播放控件
     */
    public QVPBaseVideoPlayer getCurrentPlayer() {

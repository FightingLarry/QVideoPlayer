...
import com.iamlarry.qvideoplayer.listener.IQVPVideoGifSaveListener;
...
public class GifCreateHelper {
    ...
    private StandardQVPVideoPlayer mPlayer;
    ...
    private IQVPVideoGifSaveListener mIQVPVideoGifSaveListener;
    ...
    public GifCreateHelper(StandardQVPVideoPlayer standardGSYVideoPlayer, IQVPVideoGifSaveListener IQVPVideoGifSaveListener) {
        this(standardGSYVideoPlayer, IQVPVideoGifSaveListener, 0, 1, 5, 50);
    }
    ...
    public GifCreateHelper(StandardQVPVideoPlayer standardGSYVideoPlayer, IQVPVideoGifSaveListener IQVPVideoGifSaveListener,
        ...
        mIQVPVideoGifSaveListener = IQVPVideoGifSaveListener;
        mDelay = delay;
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                if (mPicList.size() > 2) {
                    ...
                    createGif(path, mPicList, mDelay, mSampleSize, mScaleSize, mIQVPVideoGifSaveListener);
                } else {
                    mIQVPVideoGifSaveListener.result(false, null);
                }
            }
        }).start();
    }
    ...
    private void startSaveBitmap() {
        ...
        mPlayer.saveFrame(file, new IQVPVideoShotSaveListener() {
            @Override
     ...
     * @param inSampleSize            采样率，越大图片越小，越大图片越模糊，需要处理的时长越短
     * @param scaleSize               缩减尺寸比例，对生成的截图进行缩减，越大图片越模糊，需要处理的时长越短
     * @param IQVPVideoGifSaveListener 结果回调
     */
    public void createGif(File file, List<String> pics, int delay, int inSampleSize, int scaleSize,
                          final IQVPVideoGifSaveListener IQVPVideoGifSaveListener) {
        ...
        for (int i = 0; i < pics.size(); i++) {
            ...
            bitmap.recycle();
            pic.recycle();
            IQVPVideoGifSaveListener.process(i + 1, pics.size());
        }
        ...
        try {
            ...
            e.printStackTrace();
            IQVPVideoGifSaveListener.result(false, file);
            return;
        }
        IQVPVideoGifSaveListener.result(true, file);
    }
